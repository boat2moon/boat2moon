...
import MatrixRocketPlaceholder from "./MatrixRocketPlaceholder";
...
export default function RocketWordCloud() {
  ...
  return (
    <div className="relative w-full h-full">
      {/* echarts 加载中时显示 Matrix 风格二进制飞船 */}
      {!isLoaded && (
        <div className="absolute inset-0 z-10">
          <MatrixRocketPlaceholder />
        </div>
      )}
      <div
        ref={chartRef}
        className={`w-full h-full min-h-[280px] sm:min-h-[360px] lg:min-h-[500px] transition-opacity
          duration-500 ${!isLoaded ? "opacity-0" : "opacity-100"}`}
      />
      {/* 二进制尾焰效果 - 组件挂载后立刻启动 */}
      <BinaryFlame />
    </div>
